...
//            CPLEX.setParam(IloCplex.IntParam.Threads, 4);
    ...
    private void Create_C_Parameter() {
        ...
        for (int OriginIndex = 0; OriginIndex < NodesCount; OriginIndex++) {
            ...
            for (int DestinationIndex = 0; DestinationIndex < NodesCount; DestinationIndex++) {
                ...
                try {
                    C[OriginIndex][DestinationIndex] = Route.TotalCost;
                } catch (Exception e) {
            ...
            for (int t = 0; t < TaxisCount; t++) {
                for (int i = 0; i < NodesCount; i++) {
                    for (int j = 0; j < NodesCount; j++) {
                        String Name = String.format("x[%d][%d][%d][%d]", p, t, i, j);
                        double LowerBound = 0;
                        double UpperBound = 1;
                        try {
    ...
    private void Create_y_Variable() {
        ...
        for (int t = 0; t < TaxisCount; t++) {
            for (int i = 0; i < NodesCount; i++) {
                for (int j = 0; j < NodesCount; j++) {
                    String Name = String.format("y[%d][%d][%d]", t, i, j);
    ...
    private void Create_d_Variable() {
        ...
        for (int p = 0; p < PassengersCount; p++) {
            for (int t = 0; t < TaxisCount; t++) {
                String Name = String.format("d[%d][%d]", p, t);
    ...
    private void Create_u_Variable() {
        ...
        for (int i = 0; i < NodesCount; i++) {
            for (int t = 0; t < TaxisCount; t++) {
                String Name = String.format("u[%d][%d]", i, t);
...
//                for (int DownStreamIndex = 0; DownStreamIndex < NodesCount; DownStreamIndex++) {
            ...
            CPLEX.addEq(ConstraintExpr, 1, "ConstraintSet_2");
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void SetConstraintSet_03() {
        ...
        try {
            ...
            CPLEX.addEq(ConstraintExpr, 1, "ConstraintSet_3");
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void SetConstraintSet_04() {
        ...
        try {
            ...
            for (int p = 0; p < Network.SeekerPassengers.size(); p++) {
                ...
                CPLEX.addEq(ConstraintExpr, 0, "ConstraintSet_4");
            }
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void SetConstraintSet_05() {
        ...
        try {
            ...
            for (int p = 0; p < Network.SeekerPassengers.size(); p++) {
                ...
                CPLEX.addEq(ConstraintExpr, 0, "ConstraintSet_5");
            }
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void SetConstraintSet_06() {
        try {
            ...
            CPLEX.addEq(ConstraintExpr, 0, "ConstraintSet_6");
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void SetConstraintSet_07() {
        try {
            ...
            CPLEX.addLe(ConstraintExpr, 0, "ConstraintSet_7");
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void SetConstraintSet_08() {
        try {
            ...
            CPLEX.addLe(ConstraintExpr, 1, "ConstraintSet_8");
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void SetConstraintSet_09() {
        try {
            ...
            CPLEX.addLe(ConstraintExpr, 1, "ConstraintSet_9");
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void SetConstraintSet_10() {
        try {
            ...
            CPLEX.addLe(ConstraintExpr, 1, "ConstraintSet_10");
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void SetConstraintSet_11() {
        try {
            ...
            CPLEX.addLe(ConstraintExpr, 0, "ConstraintSet_11");
        } catch (IloException ex) {
            Logger.getLogger(RoutingProblem_CPLEX.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
            ...
            for (int TaxiIndex = 0; TaxiIndex < Network.Taxis.size(); TaxiIndex++) {
                for (int i = 0; i < Network.Nodes.size(); i++) {
                    for (int j = 0; j < Network.Nodes.size(); j++) {
                        ...
                        ConstraintExpr.addTerm(-Multiplier * Double.MAX_VALUE, term3);
                        double maxRideTime = U[i][j] - Double.MAX_VALUE;
                        CPLEX.addLe(ConstraintExpr, maxRideTime, String.format("ConstraintSet_12[%d][%d][%d]", TaxiIndex, i, j));
                    }
                }
            }
        ...
        try {
            ...
            for (int PassengerIndex = 0; PassengerIndex < Network.Passengers.size(); PassengerIndex++) {
                for (int TaxiIndex = 0; TaxiIndex < Network.Taxis.size(); TaxiIndex++) {
                    ...
                    int OrigintionIndex = origin.Index;
                    double Multiplier = 1;
                    IloNumVar Term1 = u[OrigintionIndex][TaxiIndex];
                    ConstraintExpr.addTerm(Multiplier, Term1);
        ...
        try {
            ...
            for (PassengerIndex = 0; PassengerIndex < Network.Passengers.size(); PassengerIndex++) {
                for (int TaxiIndex = 0; TaxiIndex < Network.Taxis.size(); TaxiIndex++) {
                    ...
                    ConstraintExpr.addTerm(Double.MAX_VALUE, term3);
                    double Number = UP[PassengerIndex] + Double.MAX_VALUE;
